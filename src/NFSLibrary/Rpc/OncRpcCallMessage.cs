...
namespace NFSLibrary.Rpc
{
    ...
    /// <para>Note that this is an abstract class. Because call message objects also
    ...
    /// the client).</para>
    /// <para>Please also note that this class implements no encoding or decoding
    ...
    public abstract class OncRpcCallMessage : NFSLibrary.Rpc.OncRpcMessage
    {
        ...
        public OncRpcCallMessage(int messageId, int program, int version, int procedure) :
            base(messageId)
        {
            messageType = NFSLibrary.Rpc.OncRpcMessageType.ONCRPC_CALL;
